...
function regulate_macchanger_auto_change_mac(){
cat << EOF
[main]
...
function _START_APACHE_AND_START_ROGUEAP_(){
	########### START APACHE2 AND START ROGUE AP
...
then
        printf "\nUpdating /etc/NetworkManager/NetworkManager.conf to avoid random mac changes in Rogue Ap\n"
        regulate_macchanger_auto_change_mac > /etc/NetworkManager/NetworkManager.conf
	printf "Please re-launch rogueap.sh\n"
	exit
fi
...
_START_APACHE_AND_START_ROGUEAP_
